...
class FastSCNN(nn.Module):
  def __init__(self, num_classes, aux=False, **kwargs):
    ...
    learning_to_downsample = LearningToDownsample(
      32, 48, 64, input_channels=kwargs.get("input_channels", 3)
    )
...
class LearningToDownsample(nn.Module):
  ...
  def __init__(self, dw_channels1=32, dw_channels2=48, out_channels=64, **kwargs):
    ...
    self.conv = _ConvBNReLU(kwargs.get("input_channels", 3), dw_channels1, 3, 2)
  ...
  if pretrained:
    if map_cpu:
      model.load_state_dict(
        torch.load(
          os.path.join(root, "fast_scnn_%s.pth" % acronyms[dataset]), map_location="cpu"
        )
      )
    else:
      model.load_state_dict(
