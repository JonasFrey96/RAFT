...
from src.segmentation.lightning_seg import Network
from src.common.ycb import get_ycb_dataloader
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--exp",
    type=file_path,
    default="cfg/exp/final/2_training_segmentation/debug/factor1_25.yml",
  ...
  if exp.get("weights_restore2", False):
    ...
    if os.path.isfile(p):
      res = torch.load(p)
      out = model.load_state_dict(res["state_dict"], strict=True)
      print("Restored weights from ckpts!")

  if exp.get("mode", "train") == "train":
    train_dataloader = get_ycb_dataloader(exp["train_dataset"], env)
    val_dataloader = get_ycb_dataloader(exp["val_dataset"], env)
    val_dataloader.dataset.deterministic_random_shuffel()
    train_dataloader.dataset.segmentation_only = True
    val_dataloader.dataset.segmentation_only = True
    ...
    test_dataloader = get_ycb_dataloader(exp["test_dataset"], env)
    trainer.test(
      model=model,
